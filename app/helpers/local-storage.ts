...
export const getSearchFromLocalStorage = () => {
    if (typeof window === 'undefined') return [];
    const searches = localStorage.getItem(LOCAL_STORAGE_SEARCHES) || JSON.stringify([]);
...
export const getDarkModeFromLocalStorage = () => {
    ...
    const darkModeRaw = localStorage.getItem(LOCAL_STORAGE_DARK_MODE) || JSON.stringify(false);
